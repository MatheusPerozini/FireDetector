...
while (cap.isOpened()):
    ...
    if ret == True:
        ...
        XcmFogo, YcmFogo, tamanhoFogo = gerarVideos(LOWER_FIRE_MASK, UPPER_FIRE_MASK, hsv, 'fogo', frame)
        rgbFogo = pegarValoresRGBImagem(frameRGB2BGR, XcmFogo, YcmFogo)
        movimentoFogo = '?'

        XcmFumaca, YcmFumaca, tamanhoFumaca = gerarVideos(LOWER_SMOKE_MASK, UPPER_SMOKE_MASK, hsv, 'fumaca', frame)
